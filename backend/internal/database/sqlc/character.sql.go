...
func (q *Queries) BulkAddArchiveCharacters(ctx context.Context, arg BulkAddArchiveCharactersParams) error {
	_, err := q.db.ExecContext(ctx, bulkAddArchiveCharacters, arg.ArchiveID, arg.Characters)
	return err
}

const bulkAddCharacters = `-- name: BulkAddCharacters :exec
;
...
func (q *Queries) BulkAddCharacters(ctx context.Context, characters interface{}) error {
	_, err := q.db.ExecContext(ctx, bulkAddCharacters, characters)
	return err
}

const bulkGetCharacters = `-- name: BulkGetCharacters :many
;

select id, name
from character
where name in (/*SLICE:characters*/?)
`

type BulkGetCharactersRow struct {
	ID   int64  `json:"id"`
	Name string `json:"name"`
}

func (q *Queries) BulkGetCharacters(ctx context.Context, characters []string) ([]BulkGetCharactersRow, error) {
	query := bulkGetCharacters
	...
	defer rows.Close()
	var items []BulkGetCharactersRow
	for rows.Next() {
		var i BulkGetCharactersRow
...
const bulkRemoveCharactersFromArchive = `-- name: BulkRemoveCharactersFromArchive :exec
;

delete from archive_character
where
    archive_id = ?
    and character_id in (select value from json_each(?2))
`

type BulkRemoveCharactersFromArchiveParams struct {
	ArchiveID  *string     `json:"archive_id"`
	Characters interface{} `json:"characters"`
}

func (q *Queries) BulkRemoveCharactersFromArchive(ctx context.Context, arg BulkRemoveCharactersFromArchiveParams) error {
	_, err := q.db.ExecContext(ctx, bulkRemoveCharactersFromArchive, arg.ArchiveID, arg.Characters)
	return err
}
...
func (q *Queries) EnsureCharacterExist(ctx context.Context, characters interface{}) ([]EnsureCharacterExistRow, error) {
	rows, err := q.db.QueryContext(ctx, ensureCharacterExist, characters)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
