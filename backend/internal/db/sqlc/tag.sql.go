...
func (q *Queries) BulkAddArchiveTags(ctx context.Context, archiveID *string) error {
	_, err := q.db.ExecContext(ctx, bulkAddArchiveTags, archiveID)
	return err
}
...
func (q *Queries) EnsureTagExist(ctx context.Context) ([]EnsureTagExistRow, error) {
	rows, err := q.db.QueryContext(ctx, ensureTagExist)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
