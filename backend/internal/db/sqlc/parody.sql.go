...
func (q *Queries) BulkAddArchiveParodies(ctx context.Context, archiveID *string) error {
	_, err := q.db.ExecContext(ctx, bulkAddArchiveParodies, archiveID)
	return err
}
...
func (q *Queries) EnsureParodyExist(ctx context.Context) ([]EnsureParodyExistRow, error) {
	rows, err := q.db.QueryContext(ctx, ensureParodyExist)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
