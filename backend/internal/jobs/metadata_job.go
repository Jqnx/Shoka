...
const JobTypeMetadata = "metadata"

type MetadataPayload struct {
	ArchiveID string
}

func NewMetadataHandler(pipeline *metadata.Pipeline, queries *sqlc.Queries, db *sql.DB, logger *slog.Logger) Handler {
	return func(ctx context.Context, job *Job) error {
		...
		return applyMetadata(ctx, queries, db, archive.ID, result)
	}
}

func applyMetadata(ctx context.Context, queries *sqlc.Queries, db *sql.DB, archiveID string, result *metadata.Result) error {
