...
const { data: filter, status } = await useAsyncData<any>(
  props.field,
  () => $fetch(`${url.value}`),
  {},
);
...
const filtered = computed(() => {
  ...
  if (props.field === "languages" || props.field === "categories") {
    const options = filter.value.filter((i: string) => !val?.includes(i));
    return search.value
      ? options.filter((option: string) => contains(option, search.value))
      : options;
  } else {
    const options = filter.value.filter(
        ...
        <div class="overflow-y-hidden h-(--reka-accordion-content-height)">
          <div class="flex flex-col gap-1 w-[98%]">
            <Button
              ...
              :key="item"
              class="justify-start bg-success hover:bg-destructive"
              size="sm"
